...
``` {r}
library("gtfs2gps")
sao <- read_gtfs("C://Code/Creating-maps-in-R/vignettes/data/gtfs.zip")
# system.file("C://Code/Creating-maps-in-R/vignettes/data/gtfs.zip", package ="gtfs2gps")
names(sao)
```
...
sao$stops
sao$shapes
sao_small <- gtfs2gps::filter_by_shape_id(sao, c(20285))
...
##tmap adding
st_test <- sf::st_geometry(sao_small_shapes_sf)
sf_test <- sf::st_geometry(sao_small_stops_sf)

library(tmap)
# source("https://github.com/Robinlovelace/geocompr/raw/master/code/08-tmshape.R", print.eval = TRUE, prompt.echo = TRUE)
source("./kml_to_tidy.R")
kableExtra::kable(tidy.kat, format = "markdown")
...
```{r}
# file.copy(paste0(tempd, "/result.zip"), file, overwrite = TRUE)
# write_gtfs(sao_small)
```
...
```{r}
sao_gps <- gtfs2gps(gtfs_data = sao_small, progress = TRUE, cores = 1, spatial_resolution = 15)
head(sao_gps)
```
