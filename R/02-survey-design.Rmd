...
Here are the reproduced plots that began the project. The orginal figures looked ...

## Data

Using data from orginal analysis as follows:

- Mice = `Data_CRinput....jan2019.R`
- Beech seed = '?'
...
# source("./R/egl_hol_map.R", echo = TRUE)
...
New Zealand's shape plotted using tmap functions.
...
nz_water = sf::st_union(nz) %>% st_buffer(22200) %>%
...
```{r nz-plot, eval=FALSE, fig.cap="Map of New Zealand"}

g = sf::st_graticule(nz, lon = c(170, 175), lat = c(-45, -40, -35))
...
MNA of mice and associated data

```{r eval=FALSE, include=FALSE}
dat <- read.csv("./data/plot-all-data1.csv")
glimpse(dat)
```

Or here is the modified script so far:

```{r eval=FALSE, include=FALSE}
###############MICE#####################
# Import data
dat <- utils::read.table("C://Code/data/Eglintonvalleydatatest1.txt", header=T, sep="\t", quote="")
# glimpse(dat)
library(lubridate)
...
## Computationally Reproduced Results (CRR)

Here are a collection of plots looking at the raw data and trends within as displayed in several previous publications.

### Raw plots {.tabset}

A collection of plots exploring MNA as done in other Ruscoe papers.
...
With my theme from publications...

```{r}
source("./R/theme_raw_fig3s.r")
p1 <- ggplot(min.mice, aes(y = n, x = trip.no, colour = grid)) +
  geom_point() +
  geom_line() +
  ggtitle("MNA") + 
  theme(legend.position = "none")

p2 <- ggplot(min.mice, aes(y = recap, x = trip.no, colour = grid)) +
  geom_point() +
  geom_line() +
  ggtitle("Recaptures") + 
...
```{r}
ggplot(min.mice, aes(y = n, x = trip.no, colour = grid.id)) +
  geom_point() +
  geom_point(aes(y = recap, colour = grid.id), alpha = 0.5,shape = "square") +
  geom_line() +
  geom_line(aes(y = recap, colour = grid.id), lty = 2) +
...
Estimating the confidence interval of these estimates are not possible as MNA is a `minumum` and therefore can only be greater than this value.

This is why we need to use a model to estimate `N` and the subsequent error associated with this estimate. Below are the abundance estimates from the CR model.
